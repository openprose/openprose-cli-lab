...
The private review bundle is not a public v1 release. Current compiled-artifact qualification covers macOS arm64 only. The sidecars do not alter existing Prose command parsing. Native acting still needs an installed Prose CLI and admitted harness. Arbitrary immutable kernel selection remains a setup limitation: the current CLI has no runtime kernel-file override, and a fixed-image build is the deterministic route. Ordinary published-on-run selection can change between readiness and action; a later digest check cannot reverse effects. This limitation must be resolved or the supported profile narrowed before a general pinned-program release claim.
