...
Quoted TOML keys are `native_max_turns="60"` and `native_timeout="8m"`; environment variables are `PROSE_NATIVE_MAX_TURNS` and `PROSE_NATIVE_TIMEOUT`. Normal flag > environment > project > user precedence applies. Zero, negative, malformed, infinite or overflowing values are rejected. Omitted values preserve the existing native launch defaults.

SDK invocation, dry-run and result records expose `nativeLimits`: `maxTurns`, `timeoutSeconds`, `toolTimeoutSeconds` (30), and `maxOutputTokens` (12000 per model request). The generic SDK harness also reports its configured limits in start and error records. These are configured allowances, not measured consumption. The harness's direct `--max-turns` and `--timeout` arguments remain available without either outer runner.
