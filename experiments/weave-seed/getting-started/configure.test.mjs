...
import {loadConfig} from '../integration/native-actor/actor.mjs';
...
test('one reviewed manifest produces valid bound configs without commands, credentials or source changes',()=>{
 ...
 const c=JSON.parse(readFileSync(result.config));assert.equal(c.timeoutMs,220000);assert.equal(c.ttlMs,300000);assert.equal(c.maxAttempts,3);assert.deepEqual(c.environmentKeys,['TYPESAFE_API_KEY','PATH','OPENAI_API_KEY']);assert.equal(result.commands.serve.at(-1),'1');
...
test('incorrect binary digest, unknown fields, outside sources and missing explicit values fail without output',()=>{
 for(const modify of [m=>m.nativeCliSha256='b'.repeat(64),m=>m.secretValue='must-not-be-accepted',m=>m.evidence=['../outside'],m=>delete m.assessment.endpoint,m=>m.maxAttempts=101,m=>m.contracts=[],m=>m.actorEnvironmentKeys=[]]) {
