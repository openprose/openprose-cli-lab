...
The existing CLI does not accept an arbitrary runtime kernel override. Its selected image must match the bound kernel and configured image digest. A caller-selected fixed-image build is the deterministic route for an arbitrary frozen kernel; an ordinary published-kernel build is suitable only when its resolved kernel is the same one. A mismatch fails; changing a JSON path does not change the CLI image. This is a current setup limitation, not an automatic package-resolution feature.
