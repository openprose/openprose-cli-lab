...
import { invokeNative, loadConfig, parseJSON, runProcess } from './actor.mjs';
...
try {
  await test('real fake CLI exact argv and selected environment, no fulfillment claim', async () => {
    ...
    assert.deepEqual(lines[0],[...common,'--dry-run','--output','json','run','program.md']); assert.deepEqual(lines[1],[...common,'--output','jsonl','run','program.md']);
  });
  ...
  await test('private phase receipts retain failures without credentials, prompts or child output', async () => {
    ...
    for(const [phase,mode,changes] of [['config','ok',{schema:2}],['input','ok',{}],['readiness','notready',{}],['revalidate','mutate',{}],['run','fail',{}],['completion','wrongkernel',{}]]) {
      ...
      await assert.rejects(invokeNative(configPath,envelope,{ambient:{FAKE_MODE:mode,OPENAI_API_KEY:'PRIVATE-API-KEY'}}), new RegExp('NATIVE_ACTOR_'+phase.toUpperCase()+'_FAILED'));
      ...
      assert.equal(record.phase,phase);assert.equal(record.errorCode,'NATIVE_ACTOR_'+phase.toUpperCase()+'_FAILED');assert.equal(record.attempt,'attempt-1');assert.equal(record.actionLaunched,['run','completion'].includes(phase));
    ...
    await assert.rejects(invokeNative(configPath,setup({receiptDirectory:receipts}),{ambient:{FAKE_MODE:'ok'},async processRunner(argv,options){const out=await runProcess(argv,options);if(!argv.includes('--dry-run'))chmodSync(receipts,0o755);return out;}}),/NATIVE_ACTOR_RECEIPT_FAILED/);chmodSync(receipts,0o700);
  });
