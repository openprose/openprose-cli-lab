...
export function processCapabilities({ assessor, actor, cwd, timeoutMs = 30000, maxOutputBytes = 1048576, environment = {} }) {
  for (const command of [assessor,actor]) {
    if (!Array.isArray(command) || !command.length || !command.every(s => typeof s === 'string' && !s.includes('\0')) || !isAbsolute(command[0])) throw Error('explicit absolute executable and argument array required');
  }
  ...
  function run(command,evidence,attempt) {
    ...
    if(result.error || result.status!==0 || result.signal || Buffer.byteLength(result.stdout??'')+Buffer.byteLength(result.stderr??'')>maxOutputBytes) throw Error('capability process failed or timed out');
